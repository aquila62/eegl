...
int main()
   ...
   /* the eegl random number generator uses the eegl algorithm */
   /* plus the Bays-Durham shuffle                             */
   eefmt *ee;                    /* eegl structure */
   ee = (eefmt *) eeglinit();    /* initialize the eegl generator */
   /*******************************************/
   ...
   *p = '\0';        /* terminate the text string */
   ...
   picrowsz = 3 * ((txtlen*15) + 80 + (eeglint(ee,40)));
   piccolsz = 24 + 40 + (eeglint(ee,40));
   ...
   while (txtindx < txtlen)
	 ...
	 while (col < 12)
	    ...
	    if (*fntp == '*')
	       ...
	       *picp++ = eeglpwr(ee,6);     /* 0-63 color depth */
	       *picp++ = eeglpwr(ee,6);     /* 0-63 color depth */
	       *picp++ = eeglpwr(ee,6);     /* 0-63 color depth */
	       } /* if asterisk in font */
	    col++;
