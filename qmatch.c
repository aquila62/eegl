/* qmatch.c - FIFO Queue Match for eegl  Version 1.0.0               */
...
/* This program uses an array of 1024 32-bit LFSRs        */
/* The period length of a 32-bit LFSR is 2^32-1.          */
/* The period length of 1024 32-bit LFSRs is the          */
/* factorial of 1024, or 5.41e2639.                       */
...
void enque(qfmt *tail, unsigned int outpt)
   ...
   newnode->prev = tail;
   /* The outpt parameter is the output of an eegl random number */
   /* generator cycle */
   newnode->outpt = outpt;
   } /* enque */
...
int main()
   ...
   unsigned int outpt;         /* 32-bit output from an eegl cycle */
   ...
   while (1)         /* infinite loop */
	 ...
	 char str[128];
         move(5,24);
         sprintf(str,"eegl with Bays-Durham Shuffle");
         addstr(str);
	 move(10,27);
